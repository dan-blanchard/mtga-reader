# Mono Scanner Port Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Port `readMtgaCards`, `readMtgaCardDatabase`, and `readMtgaInventory` to work against Arena's Mono runtime (Windows builds), testable immediately via the Wine-hosted Arena process running in CrossOver on macOS.

**Architecture:** New `src/mono/scanner.rs` module holds all Mono-specific scanner helpers (class name reading, field enumeration, heap scanning). These take `&MonoReader` (already cross-platform via `process_memory` crate) as their memory reader. The napi export wrappers in `src/napi/mod.rs` gain new `readMtgaCardsMono` / `readMtgaCardDatabaseMono` / `readMtgaInventoryMono` exports that invoke the Mono path. C# object field offsets are identical across IL2CPP and Mono (same Unity 2022.3.62f2 source), so all the layout knowledge from macOS transfers directly; only runtime-metadata access patterns change.

**Tech Stack:** Rust, `process_memory` crate (cross-platform memory reading), `MonoReader` from `src/mono_reader.rs`, napi-derive for Node.js exports.

**Key constraint — Mono `obj → class` indirection:** On IL2CPP, `class_ptr = read_ptr(obj)`. On Mono, `class_ptr = read_ptr(read_ptr(obj))` — objects store a `MonoVTable*` at +0, which then has `MonoClass*`. This extra dereference is the single most pervasive change across all scanners.

**Wine dev loop:** The Wine Arena process (PID visible via `ps aux | grep MTGA.exe`) is a real Mono Unity process running inside a Mach task. `MonoReader::new(pid)` uses `process_memory` which calls `task_for_pid` on macOS — works with sudo. We test entirely on macOS without needing a Windows machine.
...
|---|---|---|
| `src/mono/scanner.rs` | **Create** | All Mono-specific scanner helpers: heap region enumeration, class name reading, field enumeration, dict scanning, inventory scanning, string reading |
| `src/mono/mod.rs` | **Modify** | Add `pub mod scanner;` |
| `src/napi/mod.rs` | **Modify** | Add `readMtgaCardsMono`, `readMtgaCardDatabaseMono`, `readMtgaInventoryMono` napi exports |
| `try_wine_all.js` | **Create** | Smoke-test script targeting Wine Arena: calls all three Mono exports, compares against macOS IL2CPP ground truth |

---

### Task 1: Create `src/mono/scanner.rs` with core helpers

**Files:**
- Create: `src/mono/scanner.rs`
- Modify: `src/mono/mod.rs`

- [ ] **Step 1: Create the scanner module skeleton**
...
- [ ] **Step 2: Wire into `src/mono/mod.rs`**

Add at the end of `src/mono/mod.rs`:
```rust
pub mod scanner;
```
...
```bash
git add src/mono/scanner.rs src/mono/mod.rs
git commit -m "feat: add Mono scanner module for Windows Arena port

Skeleton implementation of all three readMtga*Mono functions targeting
Arena's Mono runtime. Uses MonoReader (cross-platform via process_memory
crate) for memory access and the same heap-signature-scan strategy as
the macOS IL2CPP scanners. Key differences from IL2CPP path:
- obj → class requires Mono vtable indirection (extra read_ptr)
- Class name read at MonoClass offset 0x48 instead of Il2CppClass 0x10
- Lazy class resolution during heap scan instead of __DATA pre-scan
- Pointer bounds widened for Wine's x86_64 address space
...
### Task 2: Add napi exports and test script

**Files:**
- Modify: `src/napi/mod.rs`
- Create: `try_wine_all.js`

- [ ] **Step 1: Add Mono napi exports to `src/napi/mod.rs`**

Add just before the final `read_class` napi function (around line 4700):
...
Expected: successful build, new exports visible in `index.d.ts`

- [ ] **Step 4: Run smoke test against Wine Arena**

Run: `sudo node try_wine_all.js 2>&1 | tail -40`

Expected first run: `readMtgaCardsMono` may succeed (pure data scan);
`readMtgaCardDatabaseMono` and `readMtgaInventoryMono` may need
adjustments to MonoClassField offsets or pointer bounds based on
diagnostic output.

- [ ] **Step 5: Iterate based on diagnostic output**

If any scanner fails:
1. Set `MTGA_DEBUG_MONO=1` and re-run
2. Check stderr for field dumps, candidate counts, pointer ranges
3. Adjust `mono_class_offsets`, `MonoClassField` layout constants
   (`NAME_OFF`, `OFFSET_OFF`, stride), or pointer bounds as needed
4. Re-build and re-run until all three pass

- [ ] **Step 6: Verify cross-check passes**

The smoke test's cross-check section compares Mono inventory values
against IL2CPP values (same account, cloud-synced). All wildcard counts,
gold, and gems must match exactly.

- [ ] **Step 7: Commit working Mono scanners**

```bash
git add src/mono/scanner.rs src/mono/mod.rs src/napi/mod.rs try_wine_all.js
git commit -m "feat: working Mono scanner - readMtgaCardsMono/CardDatabaseMono/InventoryMono

All three Mono-backend napi exports verified against Wine-hosted Arena
process on macOS. Cross-check against IL2CPP outputs confirms identical
account data (same wildcards, gold, gems, vault progress).
...
1. **Spec coverage**: All 3 functions (readMtgaCards, readMtgaCardDatabase, readMtgaInventory) have Mono equivalents. Heap scanning, class name verification, field offset resolution, string reading all covered.

2. **Placeholder scan**: No "TBD", "TODO", or "implement later". The MonoClassField layout has explicit constants with a documented fallback path if they're wrong (diagnostic dump + adjust).

3. **Type consistency**: `MonoFieldInfo` used consistently. `CardFieldOffsets` and `InventoryFieldOffsets` match the IL2CPP equivalents in field names. `read_mono_class_name` and `obj_to_mono_class` used consistently in all scanners.

4. **Platform handling**: Scanner uses `MonoReader` (cross-platform via process_memory) and `vmmap` (macOS). For native Windows, `find_scannable_heap_regions` will need a `VirtualQueryEx` path — deferred to a future task since we test on Wine first.

5. **Known risks**: MonoClassField layout at NAME_OFF=0x08 is from upstream code analysis, not live-verified. The diagnostic dump (`MTGA_DEBUG_MONO=1`) plus the cross-check step will catch layout mismatches on the first run.
